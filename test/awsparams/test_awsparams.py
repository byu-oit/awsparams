...
def test_get_all_parameters(fake_param, awsparams):
    next(fake_param)
    ssm = boto3.client("ssm")
    param2 = {"Name": "fakeparam2", "Type": "String", "Value": "fakevalue2"}
...
def test_get_all_parameters_path(awsparams):
    ssm = boto3.client("ssm")
    params = [
        {"Name": "/foo/bar", "Type": "String", "Value": "bar"},
        {"Name": "/foo/baz", "Type": "String", "Value": "baz"},
        {"Name": "/bar/baz", "Type": "String", "Value": "baz"},
    ]
    for param in params:
        ssm.put_parameter(**param)
    result = awsparams.get_all_parameters(prefix="/foo/", values=True)
    assert len(result) == 2
...
def test_get_all_parameters_prefix(awsparams):
    ...
    for param in params:
        ssm.put_parameter(**param)
    result = awsparams.get_all_parameters(prefix="foo.")
    assert len(result) == 2
...
def test_new_param(awsparams):
    awsparams.new_param(
        name="foo", value="bar", param_type="String", description="a test parameter"
    )
