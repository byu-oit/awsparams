...
def test_ls_values(cli_runner):
    ...
    result = cli_runner.invoke(cli.ls, ["testing.testing.", "-v"])
    cli_runner.invoke(cli.rm, ["testing.testing.testing", "-f"])
    assert result.exit_code == 0
...
def test_ls__without_decrypt(cli_runner):
    ...
    result = cli_runner.invoke(cli.ls, ["testing.testing.", "-v", "--no-decryption"])
    cli_runner.invoke(cli.rm, ["testing.testing.testing", "-f"])
    assert result.exit_code == 0
...
def test_cp_prefix(cli_runner):
    ...
    params = [
        {"Name": "/testing/testing/foo", "Value": "bar", "Type": "String"},
        {"Name": "/testing/testing/bar", "Value": "qux", "Type": "String"},
...
def test_cp_paths_single_item(cli_runner):
    ...
    result = cli_runner.invoke(cli.cp, ["/testing/testing/foo", "/newpath/foo"])
    assert result.exit_code == 0
    all_params = ssm.get_parameters_by_path(Path="/", Recursive=True)
...
def test_rm_no_params(cli_runner):
    ...
    assert result.exit_code == 0
    assert (
        "No parameters with the testing.testing. prefix found" == result.output.strip()
    )
...
def test_set_same_value(cli_runner):
    ...
    assert (
        result.output.strip()
        == "not updated, param 'testing.testing.testing' already contains that value"
    )
...
def test_cp_with_key(cli_runner):
    ssm = boto3.client("ssm")
    param = {"Name": "/foo/bar", "Value": "bar", "Type": "SecureString"}
    ssm.put_parameter(**param)
    result = cli_runner.invoke(cli.cp, ["/foo/bar", "/bar/foo", "--key", "somekmskey"])
    assert result.exit_code == 0
