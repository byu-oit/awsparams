...
@click.option('--region', type=click.STRING, help='region to run with')
...
def ls(prefix='', profile=None, region=None, values=False, decryption=True):
    """
...
@click.option('--src_region', type=click.STRING, default='', help="source region")
@click.option('--dst_profile', type=click.STRING, default='', help="destination profile")
@click.option('--dst_region', type=click.STRING, default='', help="destination region")
...
def cp(src, dst, src_profile, src_region, dst_profile, dst_region, prefix=False, overwrite=False, key=""):
    ...
    if dst_profile and dst_region and src_profile != dst_profile and not dst:
        dst = src
    elif not dst:
        click.echo(
...
@click.pass_context
def mv(ctx, src, dst, prefix=False, profile=None, region=None):
    """
    Move or rename a parameter
    """
    if prefix:
        if ctx.invoke(cp, src=src, dst=dst, src_profile=profile, src_region=region, prefix=prefix):
...
@click.option('--profile', type=click.STRING, help='alternative profile to use')
@click.option('--region', type=click.STRING, help='alternative region to use')
def rm(src, force=False, prefix=False, profile=None, region=None):
    """
...
def new(name=None, value=None, param_type='String', key='', description='', profile=None, region=None, overwrite=False):
    """
...
def set(src=None, value=None, profile=None, region=None):
    """
